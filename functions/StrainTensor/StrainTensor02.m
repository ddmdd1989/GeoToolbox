...
%%BUG%%Be careful...don't select the same staτion 3 times!!!!!! tha to lusw!
        ...
        while sel_sta~=0 
            ...
            svn(np)=svninp(sel_sta)/1000;
            sve(np)=sveinp(sel_sta)/1000;
    ...
    %Variance matrix
    for i=1:np
        VarVelMat(i,1)=svn(i)^2;
        VarVelMat(np+i,1)=sve(i)^2;
    end
%      VarVel=[sve; svn]
%      VarVel=[1 1 1 1 1 1 1 1];
%      VarVelMat=diag(VarVelMat)
%      VarVelMat=VarVel;
...
%      [E,stdE,mse,S] = lscov(ObsMatTrn,VelneMat)
    [E,stdE,mse,S] = lscov(ObsMatTrn,VelneMat,VarVelMat,'orth');
    StanDev=sqrt(mse);
    Sx=E(1);
    SxVar=stdE(1);
    Sy=E(2);
    ...
    %======================================================================
    strname=sprintf('%s%03.0fstr_par.txt',outname,treksimo);
    ...
    %--------------------------------------------------------------------------
    %circle
        cirname=sprintf('%03.0fcir.txt',treksimo);
    ...
    %--------------------------------------------------------------------------
    %ellipses
        ellname=sprintf('%03.0fell.txt',treksimo);
    ...
    %--------------------------------------------------------------------------
    %axes of ellipses
    axxname=sprintf('%03.0faxx.txt',treksimo);
...
% output file ???
cirname=sprintf('%sstr_par.OUT',outname);
...
cirname=sprintf('%sstr_par.str',outname);
outfile=fopen(fullfile(output_dir,cirname),'w');
for i=1:treksimo
    fprintf(outfile,'%5.0f %7.3f %7.3f %7.3f %7.3f  %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f  %7.3f\n',i,gmt_plot(i,1),gmt_plot(i,2),gmt_plot(i,3),gmt_plot(i,4),gmt_plot(i,5),gmt_plot(i,6),gmt_plot(i,7),gmt_plot(i,8),gmt_plot(i,9),gmt_plot(i,10),gmt_plot(i,11),gmt_plot(i,12));
end
