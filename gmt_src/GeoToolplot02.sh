...
# 	awk -F, '{print $1, $2}' $pth2inptf/ioncvel_vhor.sta | gmt psxy -Jm -O -R -St0.22c -W0.01c -Gred -K >> $outfile
	...
	if [ "$LABELS" -eq 1 ]
	then
		 awk -F, '{print $3,$2,5,0,1,"LM",$1}' $pth2sta | grep -v "PYLO" | gmt pstext -Jm -R -Dj0.2c/0.2c -Gwhite -O -K -V>> $outfile
...
# file=$pth2inptf/newvel_EUR.txt
# grep -v "#"  $file | awk '{print $3,$2,$11}' > tmpvel

# file=$pth2inptf/+040dirvel.txt
# grep -v "#"  $file | awk -F, ' NR>1 {print $3,$2,$4*-1}' > tmpvel
# # awk '{print $3,$2,$12*1000}' $pth2strain >tmpgtot
# makecpt -Cjet -T0/50/.1 > inx.cpt
# gmt	pscontour tmpvel -R -J -Wthin -Cinx.cpt -G1i/0 -O -K >> $outfile
# gmt	pscontour tmpvel -R -J -I -Cinx.cpt -O -K >> $outfile
# 
# # pscoast -J -R -W -Di -O -K -UBL/3.8c/-3.2c/"DSO-HGL/NTUA" >> $ps
# gmt	pscoast -R -J -O -K -W0.25 -Df -Na -U$logo_pos >> $outfile
# psscale -Cinx.cpt -D.9/3/3.2/0.3 -B10/:"mm/y": -I -S -O -K >> $outfile
# rm tmpvel
