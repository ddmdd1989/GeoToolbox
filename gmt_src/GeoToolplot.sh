...
function help {
	echo "/*****************************************************************/"
	echo " Program Name : GeoToolplot.sh"
	echo " Version : v-1.0"
	echo " Purpose : Plot maps for GeoToolbox"
	echo " Usage   : GeoToolplot.sh -r  |  | -o [output] | -jpg "
	echo " Switches: "
	...
	echo ""
	echo "/*** PLOT STATIONS ***********************************************/"
	echo "           -psta [:=stations] plot only stations from iput file"
	echo ""
	...
	echo " run scr: ./GeoToolplot.sh -topo -jpg"
	echo "/*************************************************************/"
	exit 1
}
...
if [ "$VVERTICAL" -eq 1 ]
then
	awk 'NR != 1 {if ($4<0) print $1,$2,$3,$4,0,0,0,$8}' $pth2vver | gmt psvelo -R -Jm -Se${VSC}/0.95/0 -W2p,red -A10p+e -Gred -O -K -L -V >> $outfile
	awk 'NR != 1 {if ($4>=0) print $1,$2,$3,$4,0,0,0,$8}' $pth2vver | gmt psvelo -R -Jm -Se${VSC}/0.95/0 -W2p,blue -A10p+e -Gblue -O -K -L -V >> $outfile
...
###scale
# echo "$vsclon $vsclat $vscmagn 0 0 0 0 $vscmagn mm" | gmt psvelo -R -Jm -Se${VSC}/0.95/10 -W2p,blue -A10p+e -Gblue -O -K -L -V >> $outfile
echo "22.48 37.16 $vscmagn 0 1 1 0 $vscmagn mm" | gmt psvelo -R -Jm -Se${VSC}/0.95/0 -W.5p,50 -A10p+e -Gblue -O -K -L -V >> $outfile
echo "22.48 37.16 $vscmagn 0 0 0 0 $vscmagn mm" | gmt psvelo -R -Jm -Se${VSC}/0.95/0 -W2p,blue -A10p+e -Gblue -O -K -L -V >> $outfile

echo "22.48 37.11 $vscmagn 0 1 1 0 $vscmagn mm" | gmt psvelo -R -Jm -Se${VSC}/0.95/0 -W.5p,50 -A10p+e -Gblue -O -K -L -V >> $outfile
echo "22.48 37.11 $vscmagn 0 0 0 0 $vscmagn mm" | gmt psvelo -R -Jm -Se${VSC}/0.95/0 -W2p,red -A10p+e -Gblue -O -K -L -V >> $outfile
echo "22.45 37.15 8 0 1 LB $vscmagn \261 1 mm/y" | gmt pstext -Jm -R -Dj-.3c/0.5c  -O -K -V>> $outfile
echo "22.48 37.16 8 0 1 RM Az = +  40\260" | gmt pstext -Jm -R -Dj0.4c/0.3c -Gwhite -O -K -V>> $outfile
echo "22.48 37.11 8 0 1 RM Az = + 130\260" | gmt pstext -Jm -R -Dj0.4c/0.3c -Gwhite -O -K -V>> $outfile
...
if [ "$GTOTAL" -eq 1 ]
then
awk '{print $3,$2,$12*1000}' $pth2strain >tmpgtot
makecpt -Cjet -T0/600/.51 > inx.cpt
gmt	pscontour tmpgtot -R -J -Wthin -Cinx.cpt -G1i/0 -O -K >> $outfile
...
psscale -Cinx.cpt -D.9/3/3.2/0.3 -B100/:"nstrain/y": -I -S -O -K >> $outfile
...
for i in `ls $pth2work*.reg`;do
	...
	awk '{if ($10>=0) print $3,$2,$10/1000000,$11/1000000}' $pth2strain | gmt psvelo -Jm $range -Sw${ROTSC}/1.e7 -Gred -E0/0/0/10 -L -A0.02  -V -K -O>> $outfile
	awk '{if ($10<0) print $3,$2,$10/1000000,$11/1000000}' $pth2strain | gmt psvelo -Jm $range -Sw${ROTSC}/1.e7 -Gblue -E0/0/0/10 -L -A0.02  -V -K -O>> $outfile
